...
public class Test 
{
	public static void main(String[] args) throws Exception	
	{
		...
		MySQL mySQL = new MySQL("192.168.20.67", "3312", "productosmn", "54wp8w7v", "mn_warehouse_cfdi32", 2, 4);

		String query = "SELECT C.*, i_anio_mes AS annio FROM cfdi32 C WHERE i_anio_mes = ?";
		...
		parametros.addPair("currentPage", 1);
		parametros.addPair("pageSize", 20);
		parametros.addPair("maxPageScrollElements", 13);
		parametros.addPair("fieldOrder", 1);
		parametros.addPair("typeOrder", "ASC");
		
		parametros.resetArray();
		
		filtro = new JSONObject();
		filtro.addPair("field", "v_rfc_emisor");
		filtro.addPair("operator", "=");		
		filtro.resetArray();
		value = new JSONObject();
		value.addPair("type", "string");
		value.addPair("value", "COS850425822");
...
//		parametros.addToArray(filtro);
		...
		parametro.addPair("value", "201606");
